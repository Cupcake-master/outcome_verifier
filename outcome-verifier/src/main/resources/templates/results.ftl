...
<div class="site-wrap">
        ...
        <div class="container">
            <div class="row align-items-end">
                <div class="col-lg-7">
                    ...
                    <p>Образовательные результаты по каждому модулю</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <table>
        ...
        <tr>
            ...
            <th>Модуль</th>
            <th>Оценка</th>
            <th>Используемые ключевые слова</th>
            <th>Неиспользуемые ключевые слова</th>
            <th>Решённые задачи корректно</th>
            <th>Нерешённые задачи корректно</th>
        </tr>
        </thead>
        <tbody>
        <tr>
        ...
        <#list results as result>
            <tr>
                <td>${result.module_id.id}</td>
                <td>${result.module_id.name}</td>
                <td>${result.grade_id.name}</td>
                <td>
                <#list result.keywordsUsed as javaKeyWordUsed>
                    ${javaKeyWordUsed.name}<#if javaKeyWordUsed_has_next>, </#if>
                </#list>
                </td>
                <td>
                <#if result.unusedKeywords??>
                    <#list result.unusedKeywords as item>
                        ${item}<#if item_has_next>, </#if>
                    </#list>
                <#else>
                    Все ключевые слова были использованны
                </#if>
                </td>
                <td>
                <#list result.solvedTasks as item>
                    ${item.id}<#if item_has_next>, </#if>
                </#list>
                </td>
                <td>
                <#list result.unSolvedTasks as item>
                    ${item.id}<#if item_has_next>, </#if>
                </#list>
                </td>
            </tr>
...
<@main name="Results &mdash; Website by OutcomeVerifier" nameCSS="developers.css"/>
